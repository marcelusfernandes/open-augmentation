...
```
Session start
├── Lê SOUL.md (.claude/rules/) + memory/ (usuário, contextos, estado atual)
...
## Executive & Status Document Standards' near other writing/documentation guidelines.\n\n## Factual Accuracy for Executive/Status Docs

* When writing executive summaries, weekly status, or committee docs, ALWAYS verify dates, PR statuses, and project facts against source documents BEFORE drafting prose.

* Never invent or assume facts (e.g., 'bot asks for password', 'no deeplink', 'observability is blind'). If unsure, investigate the code/data first.

* Do not present experimental options (e.g., Bedrock POC) as blockers or confirmed plans.

* Write FOR the stated audience (committee, C-Level) — no meta-commentary directed at the user inside the document.

## People & Attribution

* Never create a person file without a full name — ask the user if only a first name/nickname is given.

* Verify team affiliation before attributing statements or roles (e.g., empresa A vs fornecedor B vs parceiro C).

* Attribute observations to the actual speaker; do not conflate the user with other meeting participants.

## File Creation vs Inline Output

When the user requests a document (status doc, executive summary, dossier), CREATE the file on disk by default — do not deliver only inline content unless explicitly asked for a draft.
Copy
