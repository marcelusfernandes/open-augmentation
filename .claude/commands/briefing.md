...
5. **People context:** Pra cada pessoa em reunião agendada hoje, consultar ficha em `knowledge/people/<org>/<slug>.md` (via `qmd query "lex:<nome>"`). Trazer 1-2 linhas de contexto de quem é, projeto principal, padrão observado. Se a pessoa não tem ficha, mencionar como "ficha pendente — criar após primeira interação".

6. **Synthesize** into a concise briefing:
   - Current priorities (from active projects)
   - Key context from recent days (from journal/meetings)
   - Today's agenda (from calendar) com contexto de pessoas envolvidas
